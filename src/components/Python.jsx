...
export default function Python({ fullName, gmail }) {
  ...
  const showQuestion = (index) => {
    resetState();
    let currentQuestion = questionP[index];
  ...
  const handleNextButton = () => {
    ...
    if (currentQuestionIndex < questionP.length - 1) {
  ...
  const showScore = () => {
    resetState();
    setQuestionText(`Votre score est ${score} sur ${questionP.length} questions!`);
    setChoices([]);
    setCorrectAnswer(null);
    setSelectedAnswer(null);
    setShowNextButton(false);
