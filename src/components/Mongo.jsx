...
import questionM from "./questionM";
import "./styles.css";
...
export default function Quiz({ fullName, gmail }) {
  ...
  const showQuestion = (index) => {
    resetState();
    let currentQuestion = questionM[index];
  ...
  const handleNextButton = () => {
    ...
    if (currentQuestionIndex < questionM.length - 1) {
  ...
  const showScore = () => {
    resetState();
    setQuestionText(`Votre score est ${score} sur ${questionM.length} questions!`);
    setChoices([]);
    setCorrectAnswer(null);
    setSelectedAnswer(null);
    setShowNextButton(false);
