...
import questionMy from "./questionMy";
import "./styles.css";
...
export default function Mysql({ fullName, gmail }) {
  ...
  const showQuestion = (index) => {
    resetState();
    let currentQuestion = questionMy[index];
  ...
  const handleNextButton = () => {
    ...
    if (currentQuestionIndex < questionMy.length - 1) {
  ...
  const showScore = () => {
    resetState();
    setQuestionText(`Votre score est ${score} sur ${questionMy.length} questions!`);
    setChoices([]);
    setCorrectAnswer(null);
    setSelectedAnswer(null);
    setShowNextButton(false);
