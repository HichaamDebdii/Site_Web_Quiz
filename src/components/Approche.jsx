...
export default function Approche({ fullName, gmail }) {
    ...
    const showQuestion = (index) => {
        resetState();
        let currentQuestion = questionA[index];
    ...
    const handleNextButton = () => {
        ...
        if (currentQuestionIndex < questionA.length - 1) {
    ...
    const showScore = () => {
        resetState();
        setQuestionText(`Votre score est : ${score} sur ${questionA.length} questions!`);
        setChoices([]);
        setCorrectAnswer(null);
        setSelectedAnswer(null);
        setShowNextButton(false);
